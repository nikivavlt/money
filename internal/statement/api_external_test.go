...
func TestPrepareSwedbankStatement(t *testing.T) {
	...
	got, err := statement.Prepare(input, location)
	if err != nil {
		t.Fatalf(
			"statement.Prepare() returned an unexpected error: %v",
			err,
		)
	}
	...
	transaction := got.Transactions[0]
